...
// Build Catalog & UI Registry dynamically from local backend imports (forced refresh 1)
...
export default function SimulatorPage({ gamificationMode = false }) {
  ...
  const { trackComponentPlaced, trackWireDrawn, trackSimulationRun, unlockedComponents = [], coins = 0, currentLevel, currentLevelData, nextLevel, xpProgress } = useGamification()

  // ── Gamification: project context & component locking ─────────────────────
  const gamProject = useMemo(
    () => gamificationMode ? (PROJECTS.find(p => p.slug === projectName) ?? null) : null,
    [gamificationMode, projectName]
  )

  const WOKWI_TO_COMP_ID = {
    'wokwi-led': 'led',
  ...
  const isPaletteItemLocked = useCallback((itemType) => {
    ...
    return !unlockedComponents.includes(compId)
  }, [gamificationMode, unlockedComponents])

  // Gamification right panel state
  const [gamPanelOpen, setGamPanelOpen] = useState(true)
  const [gamTab, setGamTab] = useState('components')
  const [lockToast, setLockToast] = useState(null) // { label, compId }
  ...
  const showLockToast = useCallback((label, compId) => {
    ...
    lockToastTimer.current = setTimeout(() => setLockToast(null), 3500)
  }, [])
  ...
  const gamProjectComponents = useMemo(() => {
    ...
    return gamProject.components.map(c => {
      ...
      const compDef = compId ? COMPONENT_MAP[compId] : null
      const isLocked = compId ? !unlockedComponents.includes(compId) : false
      return { ...c, compId, compDef, isLocked }
    })
  }, [gamProject, unlockedComponents])

  const gamLockedCount = gamProjectComponents.filter(c => c.isLocked && c.compId).length
  const gamAllUnlocked = gamLockedCount === 0
  ...
  // Gamification mode: save circuit to sessionStorage then navigate to assessment page
  const handleGamificationSubmit = () => {
    if (!gamProject) return;
    const payload = {
      projectName: gamProject.slug,
      submittedAt: new Date().toISOString(),
      components,
      wires,
      code,
    };
    sessionStorage.setItem(`openhw_assessment_submission:${gamProject.slug}`, JSON.stringify(payload));
    navigate(`/${gamProject.slug}/assessment`);
  };
  ...
  return (
    <div style={S.page} ref={pageRef} className="min-h-screen">
                    ...
                    {paletteViewMode === 'grid' ? (
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 6, padding: '4px 0' }}>
                        {filteredItems.map(item => {
                          ...
                          const lockedCompId = locked ? WOKWI_TO_COMP_ID[item.type] : null;
                          return (
                            <div
                              key={item.type}
                              draggable={!locked}
                              onDragStart={e => !locked && onPaletteDragStart(e, item)}
                              onClick={() => { if (locked) { showLockToast(item.label, lockedCompId); } else { addComponentAtCenter(item); setSelectedPaletteItem({ ...item, group: group.group }); } }}
                              onContextMenu={e => { if (!locked) { e.preventDefault(); e.stopPropagation(); setPaletteContextMenu({ x: e.clientX, y: e.clientY, item: { ...item, group: group.group } }); } }}
                              title={locked ? `Study ${item.label} to unlock it` : item.label}
                              style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', padding: '0 4px 7px', borderRadius: 8, border: '1px solid var(--border)', background: locked ? 'rgba(0,0,0,.12)' : 'var(--card)', cursor: locked ? 'not-allowed' : 'pointer', userSelect: 'none', transition: 'all .15s', height: 104, boxSizing: 'border-box', minWidth: 0, overflow: 'hidden', position: 'relative', opacity: locked ? 0.4 : 1, filter: locked ? 'grayscale(1)' : 'none' }}
                      ...
                      filteredItems.map(item => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => { if (locked) { showLockToast(item.label, lockedCompId); } else { addComponentAtCenter(item); setSelectedPaletteItem({ ...item, group: group.group }); } }}
                            onContextMenu={e => { if (!locked) { e.preventDefault(); e.stopPropagation(); setPaletteContextMenu({ x: e.clientX, y: e.clientY, item: { ...item, group: group.group } }); } }}
                            title={locked ? `Study ${item.label} to unlock` : item.label}
                            style={{ padding: '7px 10px', borderRadius: 7, border: '1px solid var(--border)', background: locked ? 'rgba(0,0,0,.08)' : 'var(--card)', cursor: locked ? 'not-allowed' : 'pointer', userSelect: 'none', marginBottom: 4, borderLeft: `3px solid ${locked ? 'var(--border)' : groupColor}`, transition: 'all .15s', opacity: locked ? 0.4 : 1, filter: locked ? 'grayscale(1)' : 'none' }}
                          ...
                          >
                            <div style={{ fontWeight: 600, fontSize: 12, color: 'var(--text)', marginBottom: 2, display: 'flex', alignItems: 'center', gap: 5 }}>
                              {locked && <span style={{ fontSize: 10 }}>🔒</span>}
                              {item.label}
                            </div>
                            <div style={{ fontSize: 10, color: 'var(--text3)', lineHeight: 1.4 }}>
                              {locked ? 'Study theory & pass quiz to unlock' : (COMPONENT_REGISTRY[item.type]?.manifest?.description || COMPONENT_DESCRIPTIONS[item.type] || `${item.type} component`)}
                            </div>
                          </div>
                        );
                      })
      ...
      {/* LOCK TOAST — shown when clicking a locked palette item */}
      {lockToast && (
        <div style={{
          position: 'fixed', bottom: 28, left: '50%', transform: 'translateX(-50%)',
          zIndex: 9999, display: 'flex', alignItems: 'center', gap: 12,
          background: '#0d1220', border: '1px solid rgba(239,68,68,.4)',
          borderRadius: 12, padding: '13px 18px',
          boxShadow: '0 8px 32px rgba(0,0,0,.5), 0 0 0 1px rgba(239,68,68,.15)',
          fontFamily: "'Space Grotesk', sans-serif", maxWidth: 360,
          animation: 'slideInToast .25s ease',
        }}>
          <span style={{ fontSize: 20, flexShrink: 0 }}>🔒</span>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#fff', marginBottom: 2 }}>
              {lockToast.label} is locked
            </div>
            <div style={{ fontSize: 11, color: 'rgba(255,255,255,.45)' }}>
              Study the theory and pass the quiz to unlock this component.
            </div>
          </div>
          {lockToast.compId && (
            <button
              onClick={() => { setLockToast(null); navigate(`/components/${lockToast.compId}/theory`); }}
              style={{ flexShrink: 0, background: '#ef4444', border: 'none', color: '#fff', borderRadius: 7, padding: '6px 12px', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}
            >Unlock →</button>
          )}
          <style>{`@keyframes slideInToast { from { transform: translateX(-50%) translateY(10px); opacity:0; } to { transform: translateX(-50%) translateY(0); opacity:1; } }`}</style>
        </div>
      )}
